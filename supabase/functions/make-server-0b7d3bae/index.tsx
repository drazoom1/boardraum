...
// 이벤트 카드 스탯 조회 (현재 이벤트 + 전체 히스토리 집계)
app.get("/make-server-0b7d3bae/last-post-event/card-stats", async (c) => {
  try {
    ...
    // 현재 이벤트 + 히스토리 전체에서 cardUsageLog 집계
    ...
    for (const ev of [...allEvents, ...history]) {
      for (const entry of (ev.cardUsageLog || [])) {
        const key = entry.userId || entry.email || entry.userName;
        if (!key) continue;
        if (!countMap[key]) countMap[key] = { userName: entry.userName || entry.email || key, count: 0 };
        countMap[key].count++;
      }
    }
